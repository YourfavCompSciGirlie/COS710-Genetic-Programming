 ...
 * <p>A node is either:
 * <ul>
 *   <li><b>Terminal node</b> (arity = 0): a variable ("x1"–"x7") or an Ephemeral
 ...
 *
 * <p>For Structure-Based GP, this class provides {@link #prefixMatchCount(Node, Node)},
 * which implements the structural similarity primitive from the lecture slides:
...
public class Node {
    ...
    /** The operator symbol ("+", "-", "*", "/"), variable name ("x1"–"x7"),
    ...
    /**
     * Counts the number of nodes that match between two trees, traversing
     * top-down from the root and only recurring into a child pair when the
     * current node labels are equal.
     *
     * <p>This implements the structural similarity primitive described in the
     * COS 710 lecture slides: <em>"counting the number of nodes that are the
     * same from the root."</em> When two nodes at the same position share the
     * same label, they contribute 1 to the count and their children are examined
     * pairwise. As soon as node labels differ the traversal stops for that branch.
     *
     * <p>Examples:
     * <ul>
     *   <li>Two identical trees of size n return n.</li>
     *   <li>Two trees with different root labels return 0.</li>
     *   <li>Trees {@code (+ x1 x2)} and {@code (+ x1 x3)} return 2
     *       (root "+" and left child "x1" match; right children differ).</li>
     * </ul>
     *
     * @param n1 root of the first subtree (may be {@code null})
     * @param n2 root of the second subtree (may be {@code null})
     * @return the number of top-down positionally matching nodes
     */
    public static int prefixMatchCount(Node n1, Node n2) {
        if (n1 == null || n2 == null) return 0;
        // Labels must match for this position to contribute to the count
        if (!n1.label.equals(n2.label)) return 0;
        // This node matches; recurse into children pairwise
        int count = 1;
